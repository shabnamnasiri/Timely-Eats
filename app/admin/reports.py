...
def _closed_status_clause():
    # Shared WHERE clause: only count orders that are "completed"
    # (COMPLETED_ORDER_STATUS, currently "ready").
...
def get_order_report_stats(mysql):
    ...
    # Totals for completed orders: count, average prep time, revenue.
    cursor.execute(
        """
        SELECT
    ...
    # success_rate = % of all orders that ended up "completed".
...
def get_item_sales(mysql):
    # Per-item sales (volume and revenue), only counting completed orders.
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        f"""
        SELECT
...
def get_revenue_summary(mysql):
    # Total net sales from completed orders, plus a simple tax estimate.
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        """
...
def get_customer_report(mysql):
    ...
    # For each customer (role_id = 1), count and sum only their completed orders.
    cursor.execute(
        """
        SELECT
            u.user_id,
...
def get_table_report(mysql):
    ...
    # Per-table stats: completed order count/revenue/avg prep time, plus
    # the table's most-ordered item category (from completed orders).
    cursor.execute(
    """
    SELECT
        ts.table_number,
        ...
        (
            -- Subquery: most-ordered category for this table number,
            -- across all of its sessions, completed orders only.
            SELECT i.category
            FROM order_details od
            JOIN item i
    ...
    # tables is already sorted by order_count DESC, so the first row is the busiest.
    most_used = tables[0]["table_number"] if tables else "-"
    max_orders = max((t["order_count"] or 0) for t in tables) if tables else 0

    # "efficiency" = this table's order count as a % of the busiest table,
    # used to size bars in the template.
    for t in tables:
...
def register_admin_report_routes(app, mysql):
    ...
    def admin_tables():
        ...
        # Build a row per table number, showing its currently-active session
        # (if any), order count, and running total for that session.
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            """
            SELECT
