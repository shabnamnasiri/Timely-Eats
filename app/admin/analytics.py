...
def register_admin_analytics_routes(app, mysql):
    ...
    def admin_required():
        # role_id == 3 -> admin. Anyone else gets bounced to sign in.
    ...
    def admin_revenue():
        ...
        # Summary stats: total revenue across all orders, plus tax/gross.
        cursor.execute("""
            SELECT
    ...
    def admin_order_report():
        ...
        # Popular items ranked by volume (most ordered first)
        cursor.execute("""
            SELECT
                i.name                          AS item_name,
        ...
        # Daily order count for the last 7 days (for a small chart/sparkline)
        cursor.execute("""
            SELECT
    ...
    def admin_customer_report():
        ...
        # Per-customer spend + order count (role_id = 1 -> customers)
        cursor.execute("""
            SELECT
                u.user_id,
                u.username,
        ...
        # New customers who signed up this calendar month.
        cursor.execute("""
        ...
        # Average lifetime value across all customers (0 spenders included).
        avg_ltv = 0
        if customers:
            avg_ltv = round(
    ...
    def admin_table_report():
        ...
        cursor.execute("""
            SELECT
                ...
                (
                    -- Subquery: find this table's most-ordered item category.
                    SELECT i2.category
        ...
        # Tables with a session that hasn't ended yet -> currently occupied.
        cursor.execute("""
        ...
        # Most-used table number (tables list is already sorted by order_count DESC)
        most_used = tables[0][1] if tables else "-"

        # Overall avg turnover (minutes), across all completed sessions.
        cursor.execute("""
        ...
        # Max orders across tables, used to size the "efficiency" bar widths
        # in the template (so the busiest table's bar is 100%).
